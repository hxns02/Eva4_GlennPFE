...
const ClientsAdd = () => {
    ...
    const cancel = () => {
        var {clientsName, rol} = document.forms[0]; 
        var hasChanges = clientsName.value.length > 0|| rol.value.length > 0;
        if(hasChanges){
    ...
    const save = async (event) => {
        ...
        var {clientsName,  rol} = document.forms[0];
        var errors = "";
        errors += parseInt(clientsName.value) < 0 ? "Tiene que ingresar un Nombre de cliente.\n": "";
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId, "name": clientsName.value,"rol": rol.value})
              }
                            ...
                            <div className="row">
                                <div className="col-12">
                                    <div className="form-group">
                                        ...
                                        <input type="text" name="clientsName" id="clientsName" className="form-control"required />
                                    </div>
                                </div>
